...
完成的实现计划归档在 `docs/plans/archived/` 目录下。这些是**历史记录**，平时不需要阅读。仅在遇到困难、需要理解"当初为什么这样做"时参考。

- 活跃计划放在 `.claude/plans/`（Claude Code 默认位置，不入 git）
- 计划完成后，复制到 `docs/plans/archived/` 并提交到 git
- 文件名格式：`YYYY-MM-DD-<计划名>.md`
