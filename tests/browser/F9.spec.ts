...
function uniqueSlug() {
  return `f9-ui-${Date.now().toString(36)}${Math.random().toString(36).slice(2, 7)}`;
}
...
async function generateMagicLink() {
  ...
  const res = await fetch(`${supabaseUrl.replace(/\/$/, "")}/auth/v1/admin/generate_link`, {
    ...
    body: JSON.stringify({
      type: "magiclink",
      email: `f9-browser-${Date.now()}@example.com`,
...
describe("F9 audit browser smoke", () => {
  browserIt("shows audit history and expandable update diffs on the edit page", async () => {
    ...
    try {
      ...
      await page.evaluate(async (createdSlug) => {
        ...
        const create = await fetch("/api/v1/links", {
          ...
          body: JSON.stringify({
            slug: createdSlug,
            url: "https://example.com/f9/old",
          }),
        });
        if (create.status !== 201) throw new Error(`create failed: ${create.status}`);

        const first = await fetch(`/api/v1/links/${createdSlug}`, {
          method: "PATCH",
          headers: {
            "content-type": "application/json",
            ...auth,
          },
          body: JSON.stringify({ url: "https://example.com/f9/mid" }),
        });
        if (first.status !== 200) throw new Error(`first patch failed: ${first.status}`);

        const second = await fetch(`/api/v1/links/${createdSlug}`, {
          method: "PATCH",
          headers: {
            "content-type": "application/json",
            ...auth,
          },
          body: JSON.stringify({ url: "https://example.com/f9/new" }),
        });
        if (second.status !== 200) throw new Error(`second patch failed: ${second.status}`);
      }, slug);

      await page.goto(`${BASE_URL}/edit/${slug}`, { waitUntil: "networkidle0" });
      await page.waitForFunction(
        () =>
          document.body.innerText.includes("Audit log") &&
          document.body.innerText.includes("UPDATE") &&
          document.body.innerText.includes("CREATE"),
        { timeout: 20_000 },
      );
      await page.evaluate(() => {
        const button = Array.from(document.querySelectorAll("button")).find(
          (node) => node.textContent?.trim() === "Show diff",
        ) as HTMLButtonElement | undefined;
        button?.click();
      });
      await page.waitForFunction(
        () =>
          document.body.innerText.includes("https://example.com/f9/mid") &&
          document.body.innerText.includes("https://example.com/f9/new"),
        { timeout: 15_000 },
      );
