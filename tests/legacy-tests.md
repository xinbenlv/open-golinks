...
- `e2e/` — 端到端回归测试. 每个文件锁住一个已被用户揪出过的 bug, 防止再犯.
  ...
  - `F2-link-crud.test.ts` — 真实 Supabase token + Postgres 集成测试: owner CRUD、audit、url_history、软删重建、匿名限流、匿名强制 public + warning.
  ...
  - `F8-detailed-stats.test.ts` — `/api/v1/stats/query` 公开全站/单 slug scope、`/api/v1/stats/trending` public-only scope、deleted/missing 404、allowlist validation、GA4 failure downgrade.
  ...
  - `F12-browse.test.ts` — F12 Drop 决策回归: links list requireAuth、`owner=public` 拒绝、登录用户创建/恢复默认 private、不泄露其他 owner 链接.
...
- `browser/` — Puppeteer + 系统 Chrome 的生产/浏览器 smoke tests. 默认指向 Railway v2-hono URL, 可用 `BROWSER_BASE_URL` 和 `CHROME_PATH` 覆盖. F1-F8 的完整 magic-link/browser smoke 需要 `SUPABASE_URL` + `SUPABASE_SECRET_KEY`; 缺少时只跑公开页面 smoke 或跳过需登录用例.
  - `readme-tour.spec.ts` — 可选截图用例, 只有设置 `CAPTURE_README_TOUR=1` 才运行; 本地构建前端并启动 Vite preview, 将 `https://zgzg.li` 请求 proxy 到本地前端资产, mock API/SSR 数据, 通过创建/QR/edit/stats/warn 流程生成 `docs/assets/readme-tour.gif`; 需要本机 `ffmpeg`.
  ...
  - `F12.spec.ts` — 生产 privacy smoke: 无公开 browse UI、未登录 list 401、`owner=public` 400、登录用户新建 private.
...
## Proposed changes 隔离测试

[`proposals/README.md`](./proposals/README.md) 提供真实 PostgreSQL + 本地 JWT 回归、浏览器全流程与极简 diff 测试。无需生产凭据；不要与旧全局 mock.module 测试在同一进程混跑。
