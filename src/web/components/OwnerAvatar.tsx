...
export type LinkOwner = { avatarSeed: string; maskedEmail?: string | null };

export function OwnerAvatar({ owner }: { owner?: LinkOwner | null }) {
  const [dismissed, setDismissed] = useState(false);
  const tooltipId = useId();
  const label = owner?.maskedEmail || "Link owner";
  ...
  return <span className="owner-avatar" data-testid="owner-avatar" data-dismissed={dismissed}>
    <button type="button" className="owner-avatar__trigger" aria-label={owner?.maskedEmail ? `Link owner: ${label}` : label} aria-describedby={tooltipId}
    ...
    <span id={tooltipId} role="tooltip" className="owner-avatar__tooltip"><bdi>{label}</bdi></span>
  </span>;
}
