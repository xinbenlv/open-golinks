...
运行 `bun test tests/e2e/F5-claim.test.ts`、`tests/e2e/F2-link-crud.test.ts` 或 `tests/e2e/F6-warn.test.ts` 时，测试在调用 Supabase Admin `generate_link` 前失败：
...
env -u SUPABASE_SECRET_KEY bun test tests/e2e/F2-link-crud.test.ts tests/e2e/F6-warn.test.ts
```
...
- `tests/e2e/F2-link-crud.test.ts:25-59`
- `tests/e2e/F6-warn.test.ts:24-57`
