...
- **`src/web/`** - Vite + React 19 + react-router-dom v7. 详见 [`src/web/README.md`](../src/web/README.md).
  ...
  - `/dashboard` 由 `AuthGuard` 保护, 展示 owner 链接列表, 支持搜索、分页加载、Edit/Delete actions, 顶部嵌入 `ClaimBanner` 和 `StatsChart`; `StatsChart` 调 `/api/v1/stats/summary?days=364` 并用 `react-activity-calendar` 把近 52 周日点击渲染为 GitHub-style heatmap, 支持完整月份标签、1 月年份标签、hover tooltip 和 click pinned detail (`src/web/components/StatsChart.tsx:1-223`, `src/routes/api/stats.ts:10-12`, `src/lib/ga4.ts:153-159`, `package.json:24-40`).
