...
- **`src/web/`** - Vite + React 19 + react-router-dom v7. 详见 [`src/web/README.md`](../src/web/README.md).
  ...
  - `/dashboard` 由 `AuthGuard` 保护, 展示 owner 链接列表, 支持搜索、分页加载、Edit/Delete actions, 顶部嵌入 `ClaimBanner` 和 `StatsChart`; `StatsChart` 调 `/api/v1/stats/summary?days=364` 并把近 52 周日点击渲染为 GitHub-style heatmap, 支持季度/月初标签、1 月年份标签、hover/click tooltip (`src/web/components/StatsChart.tsx:4-226`, `src/routes/api/stats.ts:10-12`, `src/lib/ga4.ts:153-159`).
