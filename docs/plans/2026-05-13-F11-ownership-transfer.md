...
- 已实现 `POST /api/v1/links/:slug/transfer`, requireAuth + owner-only; recipient 按 `LOWER(email)` 查 `public.users`.
- 未注册接收方返回 `USER_NOT_FOUND`, 转给自己返回 `SELF_TRANSFER`, 非 owner 返回 `FORBIDDEN`.
- Edit 页 Danger zone 增加 transfer form + `window.confirm` 二次确认; 成功后显示 ownership transferred notice.
- TRANSFER audit 写入 `before.ownerId`, `after.ownerId`, `metadata.from_owner_id`, `metadata.to_owner_id`, `metadata.to_email`.
