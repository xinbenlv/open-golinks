...
2. 先查 `auth.users` 里是否已有这个 email。
3. 如果没有，再静默创建 Supabase Auth user。
4. 用 Supabase 返回的 `auth.users.id` 作为唯一 owner UUID。
5. 再 upsert `public.users`：
...
- 如果无法映射：
  - `links.owner_id = null`
  - 如果有真实 email，保留 `metadata.legacy_author_email`
- 如果没有可用 email：
...
- 保持并收紧 Claim Link 的 `owner_id IS NULL` 规则。
- 补 migration / claim / ACL 测试。

需要改的文档：

- 更新 `scripts/README.md`，说明新的 owner mapping 规则。
- 更新 cutover runbook，说明剩余 unowned legacy links 未被接受前不能切 DNS。

## Implementation Steps

1. 给 migration scripts 加 Supabase Admin helper。
2. 写 idempotent 的 `email -> auth.users.id` resolver。
3. 改 legacy migration，让它用 resolver 写 owner。
4. dry-run 输出：
   - 会创建哪些 auth users
   - 哪些 auth users 已存在
   - 哪些 links 会变 owned
   - 哪些 links 仍然 unowned
5. apply 模式写 users、links、metadata。
6. 补测试：
...
- Day 1: migration owner resolver + dry-run coverage report。
- Day 2: apply path + tests + docs。
...
- legacy email links 要么映射到真实 Supabase Auth UUID，要么保持 unowned/claimable。
...
- 没有可用 email 的 legacy links：保持 anonymous，即 `owner_id = null`；不做人工 CSV 批量归属。
